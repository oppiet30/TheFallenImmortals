...
include('active.php');
	...
	if(mysqli_num_rows($findForgeItems) == "4"){
		...
		if(mysqli_num_rows($findForgeItems) > "0"){
			...
			$findItems = mysqli_query($login, "SELECT * FROM inventory WHERE username='".$char['username']."' AND equipped='No' AND type='".$forgeType['type']."'");
		}else{
			$findItems = mysqli_query($login, "SELECT * FROM inventory WHERE username='".$char['username']."' AND equipped='No'");
		}
