...
if($charname == $guild['leader'] || $charname == $guild['coleader'] || $charname == $guild['captain'])
{
    ...
    elseif($upgrade == "Gold")
    {
        if($guild['bank'] >= $guild['goldcost'] && $guild['gold'] < "40")
        {
            ...
            $setguild = mysqli_query("UPDATE guilds SET gold='".$newgold."', goldcost='".$newcost."', bank='".$newbank."' WHERE name='".$guild['name']."'");
        }
    }
    elseif($upgrade == "Item Drop")
    {
        if($guild['bank'] >= $guild['itemdropcost'] && $guild['itemdrop'] < "40")
        {
            ...
            $setguild = mysqli_query("UPDATE guilds SET itemdrop='".$newitemdrop."', itemdropcost='".$newcost."', bank='".$newbank."' WHERE name='".$guild['name']."'");
        }
    }
    elseif($upgrade == "Item Boost")
    {
        if($guild['bank'] >= $guild['itemboostcost'] && $guild['itemboost'] < "40")
        {
            ...
            $setguild = mysqli_query("UPDATE guilds SET itemboost='".$newitemboost."', itemboostcost='".$newcost."', bank='".$newbank."' WHERE name='".$guild['name']."'");
        }
    }
