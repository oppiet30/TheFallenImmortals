...

{
    ...
    $findInventory = mysqli_query($login, "SELECT * FROM inventory WHERE username='".$char['username']."' AND equipped='No'");
...

{
    ...
    if($_POST['price'] > "0"){
        ...
        $findItemInInventory = mysqli_query($login, "SELECT * FROM inventory WHERE id='".$sellingID."' AND username='".$char['username']."'")or die("alert('Cannot locate item.');");
        ...
        $messageSeller = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES ('".$datestamp."', '4', 'PM', 'Chatroom', '".$message."')");

        $removeFromInventory = mysqli_query($login, "DELETE FROM inventory WHERE id='".$item['id']."'");
...

{
	...
	if($char['cash'] > "0"){
		...
		$takeCash = mysqli_query($login, "UPDATE characters SET cash=cash-'1' WHERE username='".$char['username']."'");

		$addToMarket = mysqli_query($login, "INSERT INTO trade (`fromplayer`, `itemname`, `price`) VALUES ('".$char['username']."', 'Cash', '".$_POST['price']."')")or die("alret('Failed to add item.')");
...

{
    ...
    $findItemInTrade = mysqli_query($login, "SELECT * FROM trade WHERE id='".$buyingID."'")or die("alert('Cannot locate item.');");
    ...
    if($char['gold'] >= $item['price']){
    	...
    	if($item['itemname'] == "Cash"){
    		...
    		$updateCash = mysqli_query($login, "UPDATE characters SET cash=cash+'1' WHERE username='".$char['username']."'");

    		$getGold = mysqli_query($login, "UPDATE characters SET gold=gold-'".$item['price']."' WHERE id='".$char['id']."'");

	        $giveSellerGold = mysqli_query($login, "UPDATE characters SET gold=gold+'".$item['price']."' WHERE username='".$item['fromplayer']."'");
	        ...
	        $messageSeller = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES ('".$datestamp."', '4', 'PM', '".$item['fromplayer']."', '".$message."')");

    		$removeFromTrade = mysqli_query($login, "DELETE FROM trade WHERE id='".$buyingID."'");
...

{
    ...
    $findItemInTrade = mysqli_query($login, "SELECT * FROM trade WHERE id='".$removeID."' AND fromplayer='".$char['username']."'")or die("alert('Cannot locate item.');");
    ...
    if($item['itemname'] == "Cash"){
    	...
    	$givecashback = mysqli_query($login, "UPDATE characters SET cash=cash+'1' WHERE username='".$char['username']."'");
    ...
    $removeFromTrade = mysqli_query($login, "DELETE FROM trade WHERE id='".$removeID."'");
...
$data .= "<a href=\'javascript: trade(\"\");\'>Your items in trade</a>";
	...
	$allMarket = mysqli_query($login, "SELECT * FROM trade WHERE towho='None' ORDER BY value");
