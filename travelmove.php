...
$updateMap = "False";
$findMap = mysqli_query($login, "SELECT * FROM map WHERE xpos='".$char['posx']."' and ypos='".$char['posy']."'");
$map = mysqli_fetch_assoc($findMap);
$findOre = mysqli_query($login, "SELECT * FROM ore WHERE xpos='".$char['posx']."' and ypos='".$char['posy']."'");
...
$findDemons = mysqli_query($login, "SELECT * FROM demons WHERE xpos='".$char['posx']."' and ypos='".$char['posy']."' and health>'0'");
...
if(!isset($_POST['direction'])){
	...
	if($_POST['direction'] == "up"){
		...
		if($relLoc[1]<0){
			...
			$relLoc[1] = 502;
			mysqli_query($login, "UPDATE characters SET posy=posy+'1' WHERE username='".$char['username']."'");
			$updateMap = "True";
		}
		mysqli_query($login, "UPDATE characters SET relativeLoc='".$relLoc[0].", ".$relLoc[1]."', animationSequence='".$animationSequence."' WHERE username='".$char['username']."'") or die(mysqli_error($login));
		...
		if($relLoc[0]<0){
			$relLoc[0] = 1018;
			mysqli_query($login, "UPDATE characters SET posx=posx-'1' WHERE username='".$char['username']."'");
			$updateMap = "True";
		}
		mysqli_query($login, "UPDATE characters SET relativeLoc='".$relLoc[0].", ".$relLoc[1]."', animationSequence='".$animationSequence."' WHERE username='".$char['username']."'");
	...
	if($updateMap == "True"){
		...
		$findBagDrops = mysqli_query($login, "SELECT * FROM bagdrop WHERE posx='".$char['posx']."' and posy='".$char['posy']."'");
		$bagLoc = "";
	...
	$playerFill = "";
	$time = time() - "600";
	$findPlayers = mysqli_query($login, "SELECT * FROM characters WHERE posx='".$char['posx']."' and posy='".$char['posy']."' and username<>'".$char['username']."' and lastactive>'".$time."'");
