	...
	fputs($fp, $header . $req);
		while(!feof($fp))
		{
			...
			if(strcmp ($res, "VERIFIED") == 0)
			{
				...
				$getchar = mysqli_query($conn, "SELECT * FROM characters WHERE email='".$payer_email."'") or die(mysqli_error($conn));
				$payer = mysqli_fetch_assoc($getchar);
				if($payment_amount == "5.25" && $payment_currency == "USD")
				{
					$netAmount = '5';
					$cashAmount = floor('5' * "1.2");
					$updatePlayer = mysqli_query($conn, "UPDATE characters SET networth=networth+'".$netAmount."', cash=cash+'".$cashAmount."' WHERE email='".$payer_email."'");
					...
					$query = mysqli_query($conn, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '3', '".$payer['username']."', '".$messagechat."', 'Chatroom')");
				}
