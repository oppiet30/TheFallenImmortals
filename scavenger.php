...
if($_POST['enemyid'] != NULL){
	...
	$lookforScavenger = mysqli_query($login, "SELECT * FROM scavenger WHERE username='".$char['username']."'");
	if(mysqli_num_rows($lookforScavenger) >= 1){
		print("alert('You can only accept one adventures at a time!');");
		die();
	}
	
	$findEnemy = mysqli_query($login, "SELECT * FROM enemies WHERE id='".$_POST['enemyid']."'")or die("alert('Tell the admin that you monster doesn\'t exist.');");
	...
	$addAdventure = mysqli_query($login, "INSERT INTO scavenger(`username`, `level`, `location`, `monster`, `itemname`, `collect`) VALUES('".$char['username']."', '".$level."', '".$locx.", ".$locy."', '".$enemy['name']."', '".$itemname."', '0/".$amountNeeded."')");
	...
	$openScavenger = mysqli_query($login, "SELECT * FROM scavenger WHERE username='".$char['username']."'");
...
$findScavenges = mysqli_query($login, "SELECT * FROM characters ORDER BY scavenges DESC LIMIT 50");
