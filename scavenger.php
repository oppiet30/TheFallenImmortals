...
if($_POST['enemyid'] != NULL){
	...
	$lookforScavenger = db_query("SELECT * FROM scavenger WHERE username=?", [$char['username']]);
	if(db_num_rows($lookforScavenger) >= 1){
		print("alert('You can only accept one adventures at a time!');");
		die();
	}
	
	$findEnemy = db_query("SELECT * FROM enemies WHERE id=?", [$_POST['enemyid']]);
	$enemy = db_fetch_assoc($findEnemy);
	...
	$addAdventure = db_query("INSERT INTO scavenger(`username`, `level`, `location`, `monster`, `itemname`, `collect`) VALUES(?, ?, ?, ?, ?, ?, '0/?)", [$char['username'], $level, $locx, $locy, $enemy['name'], $itemname, $amountNeeded]);
	...
	$openScavenger = db_query("SELECT * FROM scavenger WHERE username=?", [$char['username']]);
	$watchaGot = db_num_rows($openScavenger);
	if($watchaGot < 5){
		$data .= "<center><select id=\'enemylist\'>";
		$getenemies = db_query("SELECT * FROM enemies ORDER BY level");
		while($enemies = db_fetch_array($getenemies))
		{
...
$findScavenges = db_query("SELECT * FROM characters ORDER BY scavenges DESC LIMIT 50");
$data .= "<tr><td><u>User</u></td><td><u>Scavenges</u></td></tr>";
while($tscavenger = db_fetch_assoc($findScavenges)){
