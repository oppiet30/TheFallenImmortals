...
if($_POST['action'] == "use"){
	...
	if(in_array("Not Set", explode(", ", $char['presets']))){
		...
		if($preset[0] == "None"){
			...
			$one = $preset[0];
			$findCostSQL = db_query("SELECT * FROM affinity WHERE name=?", [$preset['0']]);
			$foundcost = db_fetch_assoc($findCostSQL);
		...
		if($preset[1] == "None"){
			...
			$two = $preset[1];
			$findCostSQL = db_query("SELECT * FROM affinity WHERE name=?", [$preset['1']]);
			$foundcost = db_fetch_assoc($findCostSQL);
			$spellLevel = $foundcost['level'];
			if($spellLevel == "0"){ $totalcost += '0'; }
			if($spellLevel == "1"){ $totalcost += '1000'; }
		...
		if($preset[2] == "None"){
			...
			$three = $preset[2];
			$findCostSQL = db_query("SELECT * FROM affinity WHERE name=?", [$preset['2']]);
			$foundcost = db_fetch_assoc($findCostSQL);
			$spellLevel = $foundcost['level'];
			if($spellLevel == "0"){ $totalcost += '0'; }
			if($spellLevel == "1"){ $totalcost += '1000'; }
		...
		if($preset[3] == "None"){
			...
			$four = $preset[3];
			$findCostSQL = db_query("SELECT * FROM affinity WHERE name=?", [$preset['3']]);
			$foundcost = db_fetch_assoc($findCostSQL);
			$spellLevel = $foundcost['level'];
			if($spellLevel == "0"){ $totalcost += '0'; }
			if($spellLevel == "1"){ $totalcost += '1000'; }
		...
		if($preset[4] == "None"){
			...
			$five = $preset[4];
			$findCostSQL = db_query("SELECT * FROM affinity WHERE name=?", [$preset['4']]);
			$foundcost = db_fetch_assoc($findCostSQL);
			$spellLevel = $foundcost['level'];
			if($spellLevel == "0"){ $totalcost += '0'; }
			if($spellLevel == "1"){ $totalcost += '1000'; }
		...
		if($preset[5] == "None"){
			...
			$six = $preset[5];
			$findCostSQL = db_query("SELECT * FROM affinity WHERE name=?", [$preset['5']]);
			$foundcost = db_fetch_assoc($findCostSQL);
			$spellLevel = $foundcost['level'];
			if($spellLevel == "0"){ $totalcost += '0'; }
			if($spellLevel == "1"){ $totalcost += '1000'; }
		...
		if($preset[6] == "None"){
			...
			$seven = $preset[6];
			$findCostSQL = db_query("SELECT * FROM affinity WHERE name=?", [$preset['6']]);
			$foundcost = db_fetch_assoc($findCostSQL);
			$spellLevel = $foundcost['level'];
			if($spellLevel == "0"){ $totalcost += '0'; }
			if($spellLevel == "1"){ $totalcost += '1000'; }
		...
		if($preset[7] == "None"){
			...
			$eight = $preset[7];
			$findCostSQL = db_query("SELECT * FROM affinity WHERE name=?", [$preset['7']]);
			$foundcost = db_fetch_assoc($findCostSQL);
			$spellLevel = $foundcost['level'];
			if($spellLevel == "0"){ $totalcost += '0'; }
			if($spellLevel == "1"){ $totalcost += '1000'; }
		...
		if($preset[8] == "None"){
			...
			$nine = $preset[8];
			$findCostSQL = db_query("SELECT * FROM affinity WHERE name=?", [$preset['8']]);
			$foundcost = db_fetch_assoc($findCostSQL);
			$spellLevel = $foundcost['level'];
			if($spellLevel == "0"){ $totalcost += '0'; }
			if($spellLevel == "1"){ $totalcost += '1000'; }
		...
		if($char['mana'] >= $totalcost){
			$updateblessings = db_query("UPDATE characters SET blessing=?, mana=mana-? WHERE username=?", [$preset, $totalcost, $char['username']]);
	...
	$updateCharacterPresets = db_query("UPDATE characters SET presets=? WHERE username=?", [$presetSet, $char['username']]);
