	...
	if($inventory['type'] == "Item" && $inventory['username'] == $char['username']){
		...
		if($inventory['itemname'] == "Mammons Sachel"){
			...
			if($stron == 2 && $dexon == 2 && $endon == 2 && $conon == 2 && $inton == 2){
				...
				$addtoInventory = db_query("INSERT INTO inventory (`username`, `itemname`, `type`, `equipped`, `strength`, `dexterity`, `endurance`, `concentration`, `intelligence`)VALUES (?, ?, ?, 'No', ?, ?, ?, ?, ?)", [$char['username'], $itemname, $itemtype, $newstr, $newdex, $newend, $newcon, $newint]);
				$deletesachel = db_query("DELETE FROM inventory WHERE id=?", [$_POST['itemid']]);
