		...
		if($ore['amount'] > 0){
			...
			if($char['mininglevel'] < 25){
				...
				if($char['mininglevel']+200 >= $findCopper){
					...
					$addCopper = mysqli_query("UPDATE characters SET copperore=copperore+'1' WHERE username='".$char['username']."'");
					$amount = $amount - 1;
					$minusTheOre = mysqli_query("UPDATE ore SET amount='".$amount."' WHERE id='".$ore['id']."'");
				}else{
				...
				if($randomNextLevel <= 200){
					...
					$updateMiningLevel = mysqli_query("UPDATE characters SET mininglevel=mininglevel+'1' WHERE username='".$char['username']."'");
				}
			...
			if($char['mininglevel'] >= 25 && $char['mininglevel'] < 65){
				...
				if($char['mininglevel']+200 >= $findCopper){
					...
					$addCopper = mysqli_query("UPDATE characters SET ironore=ironore+'1' WHERE username='".$char['username']."'");
					$amount = $amount - 1;
					$minusTheOre = mysqli_query("UPDATE ore SET amount='".$amount."' WHERE id='".$ore['id']."'");
				}else{
					$data .="You fail to find any ore... Keep trying, there is still some in this location!<br />";
			...
			if($char['mininglevel'] >= 65){
				...
				if($char['mininglevel']+200 >= $findSteel){
					...
					$addCopper = mysqli_query("UPDATE characters SET steelore=steelore+'1' WHERE username='".$char['username']."'");
					$amount = $amount - 1;
					$minusTheOre = mysqli_query("UPDATE ore SET amount='".$amount."' WHERE id='".$ore['id']."'");
				}
			...
			if($amount <= 0){
				...
				$updateTheOre =  mysqli_query("UPDATE ore SET xpos=".$xloc.", ypos=".$yloc.", amount='".$newAmount."' WHERE id='".$ore['id']."'");
				$data .= "That was the last of the ore here.";
			}
			
			$date = time();
			$updateLastMine = mysqli_query("UPDATE characters SET lastmine='".$date."' WHERE username='".$char['username']."'");
