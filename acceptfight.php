...
if(mysqli_num_rows($findDuel) == 0){
	...
	if($char['posx'] != "25" || $char['posy'] != "25"){
		...
		$updateCharacterLocation = mysqli_query($conn, "UPDATE characters SET posx='25', posy='25' WHERE username='".$char['username']."'");
	}
	...
	$startTheDuel = mysqli_query($conn, "UPDATE duelground SET `status`='Started', `time`='".$date."' WHERE `id`='".$duel['id']."'");
}
