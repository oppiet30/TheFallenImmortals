...
if(isset($_POST['demonid'])){
    $findDemon = db_query("SELECT * FROM demons WHERE id=?", [$_POST['demonid']]);
    if(db_num_rows($findDemon) >= "1"){
        $demon = db_fetch_assoc($findDemon);
        ...
        if($characterChance == "1" && $demon['health'] > "0"){  //Character attacks demon
            ...
            $hurtdemon = db_query("UPDATE demons SET health=health-? WHERE id=?", [$damageval, $demon['id']]);
            ...
            if($demon['health'] <= "0" || $demonNewHealth <= "0"){
                ...
                if($demon['name'] == "Barbatos"){
                    $reward = db_query("UPDATE characters SET stats=stats+'250' WHERE id=?", [$_SESSION['userid']]);
                    ...
                    $reward = db_query("UPDATE characters SET stats=stats+'1000' WHERE id=?", [$_SESSION['userid']]);
                    ...
                    $reward = db_query("UPDATE characters SET gold=gold+'1500000' WHERE id=?", [$_SESSION['userid']]);
                    ...
                    $reward = db_query("UPDATE characters SET gold=gold+'10000000' WHERE id=?", [$_SESSION['userid']]);
                    ...
                    $reward = db_query("UPDATE characters SET blood=blood+'300' WHERE id=?", [$_SESSION['userid']]);
                    ...
                    $reward = db_query("UPDATE characters SET blood=blood+'1200' WHERE id=?", [$_SESSION['userid']]);
                    ...
                    $reward = db_query("UPDATE characters SET cash=cash+'1' WHERE id=?", [$_SESSION['userid']]);
                    ...
                    $reward = db_query("UPDATE characters SET cash=cash+'3' WHERE id=?", [$_SESSION['userid']]);
                ...
                $updateCharHealth = db_query("UPDATE characters SET life=life-? WHERE id=?", [$demondamage, $_SESSION['userid']]);
                ...
                if($char['life'] <= "0" || $charhealth <= "0"){
                    ...
                    $killplayer = db_query("UPDATE characters SET life=? WHERE id=? ", [$char['endurance'], $_SESSION['userid']]);
                    $data .= "</center>";
                }else{
                    $data .= "</center>";
                }
