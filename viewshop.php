...
$checkItems = mysqli_query($login, "SELECT * FROM shop WHERE value>'10000'");
while($newLevelReq = mysqli_fetch_assoc($checkItems)){
	...
	$updateShopItems = mysqli_query($login, "UPDATE shop SET levelreq='".$newReq."' WHERE id='".$newLevelReq['id']."'")or die("alert('Unable to update levelreqs!');");
}
...
if(isset($_POST['sellid']))   //selling
{
    ...
    $check = mysqli_query($login, "SELECT * FROM inventory WHERE id='".$idinven."' AND username='".$charname."'")or die("Not an existing item in your inventory.");
    $selling = mysqli_fetch_assoc($check);
    $sellid = $selling['id'];
    $findShopThingy = mysqli_query($login, "SELECT * FROM shop WHERE itemname='".$selling['itemname']."'");
    ...
    $delete = mysqli_query($login, "DELETE FROM inventory WHERE id='".$sellid."'");
    $gold = $char['gold'] + $sellworth;
    $addgold = mysqli_query($login, "UPDATE characters SET gold='".$gold."' WHERE username='".$charname."'");
}
elseif(isset($_POST['itemid']))   //Buying
{
    $idshop = $_POST['itemid'];
    $check = mysqli_query($login, "SELECT * FROM shop WHERE id='".$idshop."'")or die("Failed to buy item.");
	...
	$querty = mysqli_query($login, "SELECT * FROM inventory WHERE username='".$charname."' AND equipped='No' ORDER BY value");
	$data .= "<option>Nothing</option>";
