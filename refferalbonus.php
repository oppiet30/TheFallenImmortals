...
if($char['refferal'] != "None" && $char['refferal'] != "" && $char['refferal'] != NULL){
    $whorefferal = mysqli_query($login, "SELECT * FROM characters WHERE username='".$char['refferal']."'");
    $refferal = mysqli_fetch_assoc($whorefferal);
    
    if($newlvl == "100"){
        $bonusGold = "10000";
        $refferalNEWgold = $refferal['gold'] + $bonusGold;
        $updateRefferal = mysqli_query($login, "UPDATE characters SET gold='".$refferalNEWgold."' WHERE username='".$char['refferal']."'");
