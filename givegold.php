...
if(isset($_POST['toUsername']) && isset($_POST['giveAmount']))
{
	...
	if(!ctype_digit($amount)){
		...
		if($char['gold'] < $amount)
		{
			...
			$findCharacter = mysqli_query($login, "SELECT * FROM characters WHERE username='".$to."'");
			...
			if($countNumRows == 0)
			{
				...
				$updateTo = mysqli_query($login, "UPDATE characters SET gold=gold-'".$amount."' WHERE username='".$char['username']."'")or die("alert('Unable to remove gold. Tell admin.');");
				$updateTo = mysqli_query($login, "UPDATE characters SET gold=gold+'".$amount."' WHERE username='".$to['username']."'")or die("alert('Unable to add gold. Tell admin.');");
				...
				$query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES ('".$datestamp."', '4', 'PM', '".$to['username']."', '".$message."')")or die("alert('Unable to insert chat log. Tell the admin!');");
			}
		}
	}
