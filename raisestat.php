...
if($statpoints >= $_POST['statAmount'] && $_POST['statAmount'] > "0" && is_numeric($_POST['statAmount']))
{
	...
	if($_POST['statChoice'] == "Strength")
	{
		...
		$raisestat = mysqli_query("UPDATE characters SET strength='".$newstr."', stats='".$newstatpoints."' WHERE id='".$_SESSION['userid']."' ") or die(mysqli_error());
	}
	elseif($_POST['statChoice'] == "Dexterity")
	{
		$newdex = $chardex + $raiseby;
		$raisestat = mysqli_query("UPDATE characters SET dexterity='".$newdex."', stats='".$newstatpoints."' WHERE id='".$_SESSION['userid']."' ") or die(mysqli_error());
	}
	elseif($_POST['statChoice'] == "Endurance")
	{
		$newend = $charend + $raiseby;
		$raisestat = mysqli_query("UPDATE characters SET endurance='".$newend."', stats='".$newstatpoints."' WHERE id='".$_SESSION['userid']."' ") or die(mysqli_error());
	}
	elseif($_POST['statChoice'] == "Intelligence")
	{
		$newint = $charint + $raiseby;
		$raisestat = mysqli_query("UPDATE characters SET intelligence='".$newint."', stats='".$newstatpoints."' WHERE id='".$_SESSION['userid']."' ") or die(mysqli_error());
	}
	elseif($_POST['statChoice'] == "Concentration")
	{
		$newcon = $charcon + $raiseby;
		$raisestat = mysqli_query("UPDATE characters SET concentration='".$newcon."', stats='".$newstatpoints."' WHERE id='".$_SESSION['userid']."' ") or die(mysqli_error());
	}
