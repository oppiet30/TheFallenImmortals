...
if($statpoints >= $_POST['statAmount'] && $_POST['statAmount'] > "0" && is_numeric($_POST['statAmount']))
{
	...
	if($_POST['statChoice'] == "Strength")
	{
		...
		$raisestat = db_query("UPDATE characters SET strength=?, stats=? WHERE id=? ", [$newstr, $newstatpoints, $_SESSION['userid']]);
	}
	elseif($_POST['statChoice'] == "Dexterity")
	{
		$newdex = $chardex + $raiseby;
		$raisestat = db_query("UPDATE characters SET dexterity=?, stats=? WHERE id=? ", [$newdex, $newstatpoints, $_SESSION['userid']]);
	}
	elseif($_POST['statChoice'] == "Endurance")
	{
		$newend = $charend + $raiseby;
		$raisestat = db_query("UPDATE characters SET endurance=?, stats=? WHERE id=? ", [$newend, $newstatpoints, $_SESSION['userid']]);
	}
	elseif($_POST['statChoice'] == "Intelligence")
	{
		$newint = $charint + $raiseby;
		$raisestat = db_query("UPDATE characters SET intelligence=?, stats=? WHERE id=? ", [$newint, $newstatpoints, $_SESSION['userid']]);
	}
	elseif($_POST['statChoice'] == "Concentration")
	{
		$newcon = $charcon + $raiseby;
		$raisestat = db_query("UPDATE characters SET concentration=?, stats=? WHERE id=? ", [$newcon, $newstatpoints, $_SESSION['userid']]);
	}
