...
$email = $_GET['email'];
$checkEmail = mysqli_query($login, "SELECT * FROM characters WHERE email='".$email."'");
$emailThere = mysqli_num_rows($checkEmail);

if($emailThere > 0 && $email != "Alex.Jezior@gmail.com"){
	$whom = mysqli_fetch_assoc($checkEmail);
	$unsubscribe = mysqli_query($login, "UPDATE characters SET subscribed='No' WHERE email='".$whom['email']."'");
