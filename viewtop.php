...
if($_POST['name'] != Null){
    ...
    $getchar = db_query("SELECT * FROM characters WHERE username=?", [$name]);
    $char = db_fetch_assoc($getchar);
    
    $getequip = db_query("SELECT * FROM inventory WHERE username=? AND equipped='Yes'", [$name]);
    ...
    if($chargold >= "1000000000000")
    {
        ...
        $setgold = db_query("UPDATE characters SET gold=? WHERE username=?", [$chargold, $name]);
    }
    $charbank = $char['bank'];
    if($charbank >= "1000000000000")
    {
        $charbank = "1000000000000";
        $setbank = db_query("UPDATE characters SET bank=? WHERE username=?", [$charbank, $name]);
    }
    ...
    $data .= "<tr>";
    $data .= "<td>";
    $findSecondClass = db_query("SELECT * FROM secondclass WHERE username=?", [$char['username']]);
    $sclass = db_fetch_assoc($findSecondClass);
...
while($char = db_fetch_array($getchar))
{
	...
	$charRank = db_num_rows($getcharRank);
	$charRank = $charRank + 1;
    $name = $char['username'];
    $level = $char['level'];
