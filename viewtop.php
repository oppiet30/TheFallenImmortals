...
if($_POST['name'] != Null){
    ...
    $getchar = mysqli_query("SELECT * FROM characters WHERE username='".$name."'") or die(mysqli_error());
    $char = mysqli_fetch_assoc($getchar);
    
    $getequip = mysqli_query("SELECT * FROM inventory WHERE username='".$name."' AND equipped='Yes'");
    ...
    if($chargold >= "1000000000000")
    {
        ...
        $setgold = mysqli_query("UPDATE characters SET gold='".$chargold."' WHERE username='".$name."'");
    }
    $charbank = $char['bank'];
    if($charbank >= "1000000000000")
    {
        $charbank = "1000000000000";
        $setbank = mysqli_query("UPDATE characters SET bank='".$charbank."' WHERE username='".$name."'");
    }
    ...
    $data .= "<tr>";
    $data .= "<td>";
    $findSecondClass = mysqli_query("SELECT * FROM secondclass WHERE username='".$char['username']."'");
...
$getchar = mysqli_query("SELECT * FROM characters WHERE status='Normal' AND level>'10'".$extention."");
while($char = mysqli_fetch_array($getchar))
{
	$getcharRank = mysqli_query("SELECT * FROM characters WHERE level>'".$char['level']."'");
