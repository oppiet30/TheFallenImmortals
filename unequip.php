...
if(mysqli_num_rows($getitem) == "1")    //Item exists
{
    ...
    if($charname == $item['username'])    //Item belongs to manipulating player
    {
        ...
        $accommodationUpdate = mysqli_query($conn, "UPDATE characters SET equipped='".$accommodations."' WHERE id='".$_SESSION['userid']."'") or die("Error updating User accommodation. Please tell the admin. " . mysqli_error($conn));        $unequip = mysqli_query($conn, "UPDATE inventory SET equipped='No' WHERE id='".$itemid."'");
        print("viewInventory();");
