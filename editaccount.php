...
if($_POST['oemail'] != NULL && $_POST['nemail'] != Null && $_POST['nemail2'] != Null){
	...
	$checkNewEmail = mysqli_query($login, "SELECT * FROM characters WHERE email='".$nemail."'");
	...
	if($oemail != $char['email']){
		...
		$addActivationDatabase = mysqli_query($login, "INSERT INTO activatenewemail (`username`, `newemail`, `verificationcode`) VALUES ('".$char['username']."', '".$nemail."', '".$randComfCode."')")or die("alert('Could not add verify code to database. Tell the admin!');");
	}
	...
	if($opass != $char['password']){
		...
		$addActivationDatabase = mysqli_query($login, "INSERT INTO activatenewpassword (`username`, `newpassword`, `verificationcode`) VALUES ('".$char['username']."', '".$npass."', '".$randComfCode."')")or die("alert('Could not add verify code to database. Tell the admin!');");
	}else{
	...
	$updateRainbow = mysqli_query($login, "UPDATE characters SET chatcolour='".$color."' WHERE id='".$_SESSION['userid']."'");
	$display .= "<font color=\'".$color."\'>Chat color has been changed.</font><br /><br />";
}
$findReferals = mysqli_query($login, "SELECT * FROM characters WHERE refferal='".$char['username']."'");
