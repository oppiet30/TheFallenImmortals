...
if(isset($_POST['charge'])){
    ...
    if($charge[1] != $spell[0] && $charge[1] != $spell[1] && $charge[1] != $spell[2] && $charge[1] != $spell[3] && $charge[1] != $spell[4] && $charge[1] != $spell[5]){
    	...
    	$NoCharge = db_query("UPDATE characters SET charge='None' WHERE id=?", [$_SESSION['userid']]);
...
elseif(isset($_POST['spellname']) && $spell[0] == $_POST['spellname'] || $spell[1] == $_POST['spellname'] || $spell[2] == $_POST['spellname'] || $spell[3] == $_POST['spellname'] || $spell[4] == $_POST['spellname'] || $spell[5] == $_POST['spellname'])
{
    ...
    if($spellName == "First Aid"){
        ...
        if($char['life'] >= $charend){
            ...
            $healCharacter = db_query("UPDATE characters SET life=? WHERE username=?", [$charHeal, $char['username']]);
            $healCharacter = db_query("UPDATE characters SET mana=? WHERE username=?", [$charMana, $char['username']]);
        ...
        if($enoughMana < 0){
            ...
            $charMana = $char['mana'] - 1000;
            $charge = "Might";
            $castCharacter = db_query("UPDATE characters SET charge=? WHERE username=?", [$charge, $char['username']]);
            $castCharacter = db_query("UPDATE characters SET mana=? WHERE username=?", [$charMana, $char['username']]);
