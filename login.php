...
$submittedPass = trim($_POST['userPass']);

$getchar = db_query("SELECT * FROM characters WHERE username=?", [$username]);
$char = db_fetch_assoc($getchar);

if (is_array($char) && password_verify($submittedPass, $char['password']))
{
	$time = time() - "700";
    $findonline = db_query("SELECT * FROM characters WHERE lastactive>? AND username=?", [$time, $char['username']]);
	$active = db_fetch_assoc($findonline);
	...
	// If a temporary password was issued via the recovery flow, and the user is
	// now logging in with it, promote it to the real password.
	if($char['temppass'] != "None" && $char['temppass'] != null && password_verify($submittedPass, $char['temppass'])){
		$newHash = password_hash($submittedPass, PASSWORD_BCRYPT);
		$resetup = db_query("UPDATE characters SET password=?, temppass='None' WHERE id=?", [$newHash, $char['id']]);
    ...
    $getbanned = db_query("SELECT * FROM banned WHERE ip=?", [$char['ip']]);
    ...
    $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES (?, '3', ?, ?, 'Chatroom')", [$date, $char['username'], $messagechat]);
            ...
            $addLogin = db_query("UPDATE characters SET logins=logins+'1' WHERE id=?", [$char['id']]);
            ...
            if($char['dailylogin'] == "0" && $char['level'] != "1"){
            	...
            	$query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES (?, '3', ?, ?, ?)", [$date, $char['username'], $messagechat, $char['username']]);
            	...
            	if($randomlogin == "1"){
            		...
            		$update = db_query("UPDATE characters SET level=level+? WHERE username=?", [$muchLevel, $char['username']]);
            		...
            		$update = db_query("UPDATE characters SET gold=gold+? WHERE username=?", [$muchgold, $char['username']]);
            		...
            		$update = db_query("UPDATE characters SET stats=stats+? WHERE username=?", [$muchstats, $char['username']]);
            		...
            		$update = db_query("UPDATE characters SET blood=blood+? WHERE username=?", [$muchblood, $char['username']]);
            		$inform = "<b>You gain ".number_format($muchblood)." Blood for logging in today!</b><br />";
            	}
            	$query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES (?, '3', ?, ?, ?)", [$date, $char['username'], $inform, $char['username']]);
            	$addDailyLogin = db_query("UPDATE characters SET dailylogin='1' WHERE username=?", [$char['username']]);
