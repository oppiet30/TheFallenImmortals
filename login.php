...
$getcharold = mysqli_query($login, "SELECT * FROM characters WHERE username='".$username."' AND password='".$passwordold."' ");
$getchar = mysqli_query($login, "SELECT * FROM characters WHERE username='".$username."' AND password='".$password."' OR temppass='".$password."'");
...
if(mysqli_num_rows($getcharold) === 1)
{
	...
	$addNewPassword = mysqli_query($login, "UPDATE characters SET password='".$password."' WHERE username='".$char['username']."'");
...
elseif(mysqli_num_rows($getchar) === 1)
{
	...
	$time = time() - "700";
    $findonline = mysqli_query($login, "SELECT * FROM characters WHERE lastactive>'".$time."' AND username='".$char['username']."'");
    ...
    $getbanned = mysqli_query($login, "SELECT * FROM banned WHERE ip='".$char['ip']."'");
    ...
    else
    {
        if($char['activated'] == "Yes" || $char['level'] < "100")
        {
			...
			if($char['temppass'] != "None" && $char['temppass'] == $password){
				$resetup = mysqli_query($login, "UPDATE characters SET password='".$password."', temppass='None' WHERE id='".$char['id']."'");
    ...
    $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '3', '".$char['username']."', '".$messagechat."', 'Chatroom')");
            ...
            $addLogin = mysqli_query($login, "UPDATE characters SET logins=logins+'1' WHERE id='".$char['id']."'");
            ...
            if($char['dailylogin'] == "0" && $char['level'] != "1"){
            	...
            	$query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '3', '".$char['username']."', '".$messagechat."', '".$char['username']."')");
            	...
            	if($randomlogin == "1"){
            		...
            		$update = mysqli_query($login, "UPDATE characters SET level=level+'".$muchLevel."' WHERE username='".$char['username']."'");
            		...
            		$update = mysqli_query($login, "UPDATE characters SET gold=gold+'".$muchgold."' WHERE username='".$char['username']."'");
            		...
            		$update = mysqli_query($login, "UPDATE characters SET stats=stats+'".$muchstats."' WHERE username='".$char['username']."'");
            		...
            		$update = mysqli_query($login, "UPDATE characters SET blood=blood+'".$muchblood."' WHERE username='".$char['username']."'");
            		$inform = "<b>You gain ".number_format($muchblood)." Blood for logging in today!</b><br />";
            	}
            	$query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '3', '".$char['username']."', '".$inform."', '".$char['username']."')");
            	$addDailyLogin = mysqli_query($login, "UPDATE characters SET dailylogin='1' WHERE username='".$char['username']."'");
