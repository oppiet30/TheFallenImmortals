<?php
	...
	$char = mysqli_fetch_assoc($getchar)or die(mysqli_error($login));
	...
	if($char['votecompleted'] <= time()){
		...
		if($_POST['reward'] != NULL && $char['vote'] == "Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes"){
			...
			if($_POST['reward'] == "gold"){
                ...
                $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '3', '".$char['username']."', '".$messagechat."', 'Chatroom')")or die(mysqli_error($login));

				$display .= "<font size=\'14px\'><center>You recieve 5,000 gold for voting!</center></font>";

				$newgold = $char['gold'] + "5000";

				$updateGold = mysqli_query($login, "UPDATE characters SET gold='".$newgold."' WHERE id='".$_SESSION['userid']."'")or die(mysqli_error($login));
				...
				$updateStatpoints = mysqli_query($login, "UPDATE characters SET stats='".$newstatpoints."' WHERE id='".$_SESSION['userid']."'")or die(mysqli_error($login));
				...
				$updateBlood = mysqli_query($login, "UPDATE characters SET blood='".$newblood."' WHERE id='".$_SESSION['userid']."'")or die(mysqli_error($login));
			...
			if($consecutivevotemax >= time() || $char['consecutivevotes'] == 0){
				...
				if($newconsecutive == 14){
					$givecc = mysqli_query($login, "UPDATE characters SET cash=cash+'1' WHERE id='".$_SESSION['userid']."'")or die(mysqli_error($login));
					$newconsecutive = 0;
			...
			$updateVotes = mysqli_query($login, "UPDATE characters SET consecutivevotes='".$newconsecutive."' WHERE id='".$_SESSION['userid']."'")or die(mysqli_error($login));

			

			$newtime = time() + 86400;

			$updateTime = mysqli_query($login, "UPDATE characters SET votecompleted='".$newtime."' WHERE id='".$_SESSION['userid']."'")or die(mysqli_error($login));
		    ...
		    $updateVote = mysqli_query($login, "UPDATE characters SET vote='".$newVote."' WHERE id='".$_SESSION['userid']."'");
	...
	if($char['vote'] == "Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes" && $char['votecompleted'] <= time() && $char['votecompleted'] != "0"){
		...
		$updatevotes = mysqli_query($login, "UPDATE characters SET vote='".$newvotes."' WHERE id='".$_SESSION['userid']."'")or die("PROBREM!");
