<?php
	...
	$char = db_fetch_assoc($getchar)or die(db_error());
	...
	if($char['votecompleted'] <= time()){
		...
		if($_POST['reward'] != NULL && $char['vote'] == "Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes"){
			...
			if($_POST['reward'] == "gold"){
				...
				$updateGold = db_query("UPDATE characters SET gold=? WHERE id=?", [$newgold, $_SESSION['userid']]);
				...
				$updateStatpoints = db_query("UPDATE characters SET stats=? WHERE id=?", [$newstatpoints, $_SESSION['userid']]);
				...
				$updateBlood = db_query("UPDATE characters SET blood=? WHERE id=?", [$newblood, $_SESSION['userid']]);
			...
			if($consecutivevotemax >= time() || $char['consecutivevotes'] == 0){
				...
				if($newconsecutive == 14){
					$givecc = db_query("UPDATE characters SET cash=cash+'1' WHERE id=?", [$_SESSION['userid']]);
					$newconsecutive = 0;
			...
			$updateVotes = db_query("UPDATE characters SET consecutivevotes=? WHERE id=?", [$newconsecutive, $_SESSION['userid']]);

			

			$newtime = time() + 86400;

			$updateTime = db_query("UPDATE characters SET votecompleted=? WHERE id=?", [$newtime, $_SESSION['userid']]);
		    ...
		    $updateVote = db_query("UPDATE characters SET vote=? WHERE id=?", [$newVote, $_SESSION['userid']]);
	...
	if($char['vote'] == "Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes, Yes" && $char['votecompleted'] <= time() && $char['votecompleted'] != "0"){
		...
		$updatevotes = db_query("UPDATE characters SET vote=? WHERE id=?", [$newvotes, $_SESSION['userid']]);
