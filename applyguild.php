...
if($chargold >= "1000000")    //1m
{
    ...
    $getguild = mysqli_query($login, "SELECT * FROM guilds WHERE id='".$guildname."'");
    if(mysqli_num_rows($getguild) == "1")
    {
        $guild = mysqli_fetch_assoc($getguild);
        $getmembers = mysqli_query($login, "SELECT * FROM characters WHERE guild='".$guild['name']."'");
        ...
        if($guild['recruiting'] == "Yes" && $members < "10")
        {
            if($guild['accept'] == "Approve")
            {
                ...
                $updatechar = mysqli_query($login, "UPDATE characters SET gold='".$newgold."' WHERE id='".$_SESSION['userid']."'");
                $setapplication = mysqli_query($login, "INSERT INTO applications (`guild`, `username`) VALUES ('".$guild['name']."', '".$charname."')");
            }
            elseif($guild['accept'] == "Auto")
            {
                ...
                $newgold = $chargold - "1000000";
                $updatechar = mysqli_query($login, "UPDATE characters SET gold='".$newgold."', guild='".$guild['name']."' WHERE id='".$_SESSION['userid']."'");
            }
        }
        else
        {
