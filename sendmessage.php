...
if($char['username'] != NULL || $char['username'] != "")
{
    ...
    else
    {
        if(substr($message, 0, 3) == "/a " && $char['userlevel'] == "1")    //Announcement
        {
            ...
            $sendIndex = db_query("INSERT INTO announcements (`announcement`) VALUES (?)", [$announcementPost]);
            $message = "<strong><font color=\'#FFAA00\'>Announcement: ".$message."</font></strong><br />";
            $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES (?, '3', ?, ?, 'Chatroom')", [$date, $charname, $message]);
        }
        ...
        elseif(substr($message, 0, 8) == "/remove " && $char['userlevel'] == "1")    //Removing characters from the game
        {
            ...
            $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
            VALUES (?, '3', ?, ?, 'Chatroom')", [$date, $char['username'], $message]);

            $query = db_query("DELETE FROM characters WHERE username=? ", [$removeuser]);
        }
        elseif(substr($message, 0, 3) == "/id")
        {
            $player = substr($message, 4);
            $readidplayer = db_query("SELECT * FROM characters WHERE username=? ", [$player]);
            $idplayer = db_fetch_assoc($readidplayer);
            ...
            $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`)
            VALUES (?, '4', ?, ?, ?)", [$date, $char['username'], $char['username'], $iduser]);
        }
        elseif(substr($message, 0, 3) == "/ip" && $char['userlevel'] < "3")
        {
            ...
            $data = "";
            $num = "1";
            $getips = db_query("SELECT * FROM characters WHERE ip=?", [$checkip]);
            while($ips = db_fetch_array($getips))
            {
                if($num < db_num_rows($getips))
                {
                    $data .= $ips['username'].", ";
                }
                else
            ...
            $message = "<b><font color=\'#DD0000\'>".db_num_rows($getips)." Results for (".$checkip."): ".$data."</font></b><br />";
            $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
            VALUES (?, '4', 'PM', ?, ?)", [$date, $message, $char['username']]);
        }
        elseif(substr($message, 0, 5) == "/warn" && $char['userlevel'] < "3")
        {
            ...
            $query = db_query("SELECT * FROM warnings WHERE username=? ", [$warnuser]);
            $userwarning = db_fetch_assoc($query);

            $newwarning = $userwarning['warning'] + "1";
            $query = db_query("UPDATE warnings SET warnings=? WHERE username=? ", [$newwarning, $warnuser]);
        }
        elseif(substr($message, 0, 4) == "/mod" && $char['userlevel'] < "3")
        {
            ...
            $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
            VALUES (?, '4', ?, ?, 'Mod')", [$date, $char['username'], $message]);
        }
        elseif(substr($message, 0, 9) == "/givegold" && $char['userlevel'] == "1")
        {
            ...
            if($goldstr['0'] != "" && $goldstr['1'] != "" && $goldstr['2'] != "")
            {
                ...
                $qwerygold = db_query("UPDATE characters SET gold=gold+? WHERE username=?", [$goldamount, $usergive]);

                $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
                VALUES (?, '3', ?, ?, 'Chatroom')", [$date, $char['username'], $goldmessage]);
            }
            else
            {
        ...
        elseif(substr($message, 0, 9) == "/givecash" && $char['userlevel'] == "1")
        {
            ...
            if($cashstr['0'] != "" && $cashstr['1'] != "" && $cashstr['2'] != "")
            {
                ...
                $qwerycash = db_query("UPDATE characters SET cash=cash+?, networth=networth+? WHERE username=?", [$newcash, $newcash, $usergive]);

                $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
                VALUES (?, '3', ?, ?, 'Chatroom')", [$date, $char['username'], $cashmessage]);
            }
            else
            {
        ...
        elseif(substr($message, 0, 5) == "/mute" && $char['userlevel'] < "3")
        {
            ...
            if($mutestr['0'] != "" && $mutestr['1'] != "" && $mutestr['2'] != "")
            {
                ...
                $query = db_query("INSERT INTO muted (`username`, `mutedby`, `mutetime`)
                VALUES (?, ?, ?)", [$mutestr['1'], $char['username'], $mutetime]);

                $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
                VALUES (?, '3', ?, ?, 'Chatroom')", [$date, $char['username'], $mutemessage]);
            }
            else
            {
        ...
        elseif(substr($message, 0, 7) == "/unmute" && $char['userlevel'] < "3")
        {
            ...
            $query = db_query("DELETE FROM muted WHERE username=? ", [$unmuteuser]);

            $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
            VALUES (?, '3', ?, ?, 'Chatroom')", [$date, $char['username'], $unmutemessage]);
        }
        elseif(substr($message, 0, 8) == "/suspend" && $char['userlevel'] <= "2")
        {
            ...
            if($mutestr['0'] != "" && $mutestr['1'] != "" && $mutestr['2'] != "" && $mutestr['3'] != "")
            {
                ...
                $setstatus = db_query("UPDATE characters SET status='Suspended', lastactive='0', endsuspend=?, reason=? WHERE username=?", [$suspendtill, $mutestr['3'], $mutestr['1']]);

                $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
                VALUES (?, '3', ?, ?, 'Chatroom')", [$date, $char['username'], $suspendmessage]);
            }
            else
            {
        ...
        elseif(substr($message, 0, 4) == "/ban" && $char['userlevel'] < "2")
        {
            ...
            $getbanned = db_query("SELECT * FROM characters WHERE username=?", [$banuser]);
            $banned = db_fetch_assoc($getbanned);
            $banip = $banned['ip'];

            $query = db_query("INSERT INTO banned (`ip`)
            VALUES (?)", [$banip]);

            $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
            VALUES (?, '3', ?, ?, 'Chatroom')", [$date, $char['username'], $banmessage]);
        }
        elseif(substr($message, 0, 2) == "/e")
        {
            $query = db_query("SELECT username FROM muted WHERE username=? ", [$char['username']]);
            ...
            else
            {
                ...
                $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
                VALUES (?, '3', ?, ?, 'Chatroom')", [$date, $char['username'], $message1]);
            }
        }
        ...
        elseif(substr($message, 0, 3) == "/m ")
        {
            ...
            else
            {
                ...
                {
                	$whatTimeIsIt = time();
                	$findTo = db_query("SELECT * FROM characters WHERE username=?", [$touser]);
                	$toTime = db_fetch_assoc($findTo);
                	...
                	if($whatTimeIsIt > $charLastActive){
	                    ...
	                    $query = db_query("INSERT INTO chatroommessage (`date`, `userlevel`, `username`, `to`, `message`) VALUES (?, '4', 'PM', ?, ?)", [$datestamp, $touser, $message3]);
	                    
	                    $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES (?, '4', 'PM', ?, ?)", [$datestamp, $touser, $message3half]);
	
	                    $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES (?, '4', 'PM', ?, ?)", [$datestamp, $char['username'], $message4]);
                	}else{
	                    $message3 = "<font color=\'#FF7700\'>From </font><a href=\'javascript:toptell(\"".$char['username']."\");\'><font color=\'#FF7700\' style=\'text-decoration:none\'>".$charname."</font></a><font color=\'#FF7700\'>: ".$message2." [".$date."]</font><br />";
	                    $message4 = "<font color=\'#FF7700\'>To </font><a href=\'javascript:toptell(\"".$touser."\");\'><font color=\'#FF7700\' style=\'text-decoration:none\'>".$touser."</font></a><font color=\'#FF7700\'>: ".$message2." [".$date."]</font><br />";
	
	                    $query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES (?, '4', 'PM', ?, ?)", [$datestamp, $touser, $message3]);
        ...
        elseif(substr($message, 0, 13) == "/castaffinity")
        {
            ...
            if($caststr[1] != NULL && $caststr[2] != NULL){
            	$findCharacter = db_query("SELECT * FROM characters WHERE username=?", [$caststr[1]]);
            	if(db_num_rows($findCharacter) > 0){
            		if($caststr[2] == "Might" || $caststr[2] == "Might II" || $caststr[2] == "Might III" || $caststr[2] == "Might IV" || $caststr[2] == "Might V" || $caststr[2] == "Speed" || $caststr[2] == "Speed II" || $caststr[2] == "Speed III" || $caststr[2] == "Speed IV" || $caststr[2] == "Speed V" || $caststr[2] == "Constitution" || $caststr[2] == "Constitution II" || $caststr[2] == "Constitution III" || $caststr[2] == "Constitution IV" || $caststr[2] == "Constitution V" || $caststr[2] == "Intelligence" || $caststr[2] == "Intelligence II" || $caststr[2] == "Intelligence III" || $caststr[2] == "Intelligence IV" || $caststr[2] == "Intelligence V" || $caststr[2] == "Concentration" || $caststr[2] == "Concentration II" || $caststr[2] == "Concentration III" || $caststr[2] == "Concentration IV" || $caststr[2] == "Concentration V"){
            			if(in_array($caststr[2], $spell)){
            				...
            				if($char['mana'] >= $manaCost){
            					$castto = db_fetch_assoc($findCharacter);
            					...
            					$giveUserAffinity = db_query("UPDATE characters SET blessing=? WHERE username=?", [$newBlessingString, $caststr[1]]);
            					$subtractMana = db_query("UPDATE characters SET mana=mana-? WHERE id=?", [$manaCost, $char['id']]);
								...
								$query = db_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES (?, '4', 'PM', ?, ?)", [$datestamp, $castto['username'], $message]);
        ...
        {
            $query = db_query("SELECT * FROM muted WHERE username=?", [$char['username']]);
            ...
            else
            {
            	...
            	$preGetGuild = db_query("SELECT * FROM guilds WHERE name=?", [$char['guild']]);
            	$getGuild = db_fetch_assoc($preGetGuild);
                ...
                $userStatus = "".$getGuildTag."-<font color=\'red\'>Moderator</font>";
                    if($char['chatcolour'] == "00DDDD"){
                     $modupdate = db_query("UPDATE characters SET chatcolour='BBBBBB' WHERE username=?", [$char['username']]);
                    }
            ...
            $query = db_query("UPDATE characters SET lastactive=? WHERE username=? ", [$date, $char['username']]);
        }
    }
}
else
