...
if($char['username'] != NULL || $char['username'] != "")
{
    ...
    else
    {
        if(substr($message, 0, 3) == "/a " && $char['userlevel'] == "1")    //Announcement
        {
            ...
            $sendIndex = mysqli_query($login, "INSERT INTO announcements (`announcement`) VALUES ('".$announcementPost."')");
            $message = "<strong><font color=\'#FFAA00\'>Announcement: ".$message."</font></strong><br />";
            $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '3', '".$charname."', '".$message."', 'Chatroom')");
        }
        ...
        elseif(substr($message, 0, 8) == "/remove " && $char['userlevel'] == "1")    //Removing characters from the game
        {
            ...
            VALUES ('".$date."', '3', '".$char['username']."', '".$message."', 'Chatroom')") or die(mysqli_error($login));

            $query = mysqli_query($login, "DELETE FROM characters WHERE username='".$removeuser."' ");
        }
        elseif(substr($message, 0, 3) == "/id")
        {
            $player = substr($message, 4);
            $readidplayer = mysqli_query($login, "SELECT * FROM characters WHERE username='".$player."' ") or die(mysqli_error($login));
            ...
            $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`)
            VALUES ('".$date."', '4', '".$char['username']."', '".$char['username']."', '".$iduser."')") or die(mysqli_error($login));
        }
        elseif(substr($message, 0, 3) == "/ip" && $char['userlevel'] < "3")
        {
            ...
            $data = "";
            $num = "1";
            $getips = mysqli_query($login, "SELECT * FROM characters WHERE ip='".$checkip."'");
            ...
            $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
            VALUES ('".$date."', '4', 'PM', '".$message."', '".$char['username']."')") or die(mysqli_error($login));
        }
        elseif(substr($message, 0, 5) == "/warn" && $char['userlevel'] < "3")
        {
            ...
            $query = mysqli_query($login, "SELECT * FROM warnings WHERE username='".$warnuser."' ");
            $userwarning = mysqli_fetch_assoc($query);

            $newwarning = $userwarning['warning'] + "1";
            $query = mysqli_query($login, "UPDATE warnings SET warnings='".$newwarning."' WHERE username='".$warnuser."' ");
        }
        elseif(substr($message, 0, 4) == "/mod" && $char['userlevel'] < "3")
        {
            ...
            $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
            VALUES ('".$date."', '4', '".$char['username']."', '".$message."', 'Mod')") or die(mysqli_error($login));
        }
        elseif(substr($message, 0, 9) == "/givegold" && $char['userlevel'] == "1")
        {
            ...
            if($goldstr['0'] != "" && $goldstr['1'] != "" && $goldstr['2'] != "")
            {
                ...
                $qwerygold = mysqli_query($login, "UPDATE characters SET gold=gold+'".$goldamount."' WHERE username='".$usergive."'");

                $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
                VALUES ('".$date."', '3', '".$char['username']."', '".$goldmessage."', 'Chatroom')") or die(mysqli_error($login));
            }
            else
            {
        ...
        elseif(substr($message, 0, 9) == "/givecash" && $char['userlevel'] == "1")
        {
            ...
            if($cashstr['0'] != "" && $cashstr['1'] != "" && $cashstr['2'] != "")
            {
                ...
                $qwerycash = mysqli_query($login, "UPDATE characters SET cash=cash+'".$newcash."', networth=networth+'".$newcash."' WHERE username='".$usergive."'");

                $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
                VALUES ('".$date."', '3', '".$char['username']."', '".$cashmessage."', 'Chatroom')") or die(mysqli_error($login));
            }
            else
            {
        ...
        elseif(substr($message, 0, 5) == "/mute" && $char['userlevel'] < "3")
        {
            ...
            if($mutestr['0'] != "" && $mutestr['1'] != "" && $mutestr['2'] != "")
            {
                ...
                $query = mysqli_query($login, "INSERT INTO muted (`username`, `mutedby`, `mutetime`)
                VALUES ('".$mutestr['1']."', '".$char['username']."', '".$mutetime."')") or die(mysqli_error($login));

                $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
                VALUES ('".$date."', '3', '".$char['username']."', '".$mutemessage."', 'Chatroom')") or die(mysqli_error($login));
            }
            else
            {
        ...
        elseif(substr($message, 0, 7) == "/unmute" && $char['userlevel'] < "3")
        {
            ...
            $query = mysqli_query($login, "DELETE FROM muted WHERE username='".$unmuteuser."' ") or die(mysqli_error($login));

            $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
            VALUES ('".$date."', '3', '".$char['username']."', '".$unmutemessage."', 'Chatroom')") or die(mysqli_error($login));
        }
        elseif(substr($message, 0, 8) == "/suspend" && $char['userlevel'] <= "2")
        {
            ...
            if($mutestr['0'] != "" && $mutestr['1'] != "" && $mutestr['2'] != "" && $mutestr['3'] != "")
            {
                ...
                $setstatus = mysqli_query($login, "UPDATE characters SET status='Suspended', lastactive='0', endsuspend='".$suspendtill."', reason='".$mutestr['3']."' WHERE username='".$mutestr['1']."'");

                $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
                VALUES ('".$date."', '3', '".$char['username']."', '".$suspendmessage."', 'Chatroom')") or die(mysqli_error($login));
            }
            else
            {
        ...
        elseif(substr($message, 0, 4) == "/ban" && $char['userlevel'] < "2")
        {
            ...
            $getbanned = mysqli_query($login, "SELECT * FROM characters WHERE username='".$banuser."'");
            $banned = mysqli_fetch_assoc($getbanned);
            $banip = $banned['ip'];

            $query = mysqli_query($login, "INSERT INTO banned (`ip`)
            VALUES ('".$banip."')") or die(mysqli_error($login));

            $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
            VALUES ('".$date."', '3', '".$char['username']."', '".$banmessage."', 'Chatroom')") or die(mysqli_error($login));
        }
        elseif(substr($message, 0, 2) == "/e")
        {
            $query = mysqli_query($login, "SELECT username FROM muted WHERE username='".$char['username']."' ") or die(mysqli_error($login));
            ...
            else
            {
                ...
                $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)
                VALUES ('".$date."', '3', '".$char['username']."', '".$message1."', 'Chatroom')") or die(mysqli_error($login));
            }
        }
        elseif(substr($message, 0, 7) == "/guild ")
        {
            ...
            else
            {
                ...
                if($message != "")
                {
                    ...
                    $getmembers = mysqli_query($login, "SELECT * FROM characters WHERE guild='".$charguild."'");
                    while($member = mysqli_fetch_array($getmembers))
                    {
                        $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES ('".$timestamp."', '4', 'PM', '".$member['username']."', '".$message."')");
                    }
                }
            }
        }
        elseif(substr($message, 0, 3) == "/m ")
        {
            ...
            else
            {
                ...
                {
                	$whatTimeIsIt = time();
                	$findTo = mysqli_query($login, "SELECT * FROM characters WHERE username='".$touser."'");
                	...
                	if($whatTimeIsIt > $charLastActive){
	                    ...
	                    $query = mysqli_query($login, "INSERT INTO chatroommessage (`date`, `userlevel`, `username`, `to`, `message`) VALUES ('".$datestamp."', '4', 'PM', '".$touser."', '".$message3."')");
	                    
	                    $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES ('".$datestamp."', '4', 'PM', '".$touser."', '".$message3half."')");
	
	                    $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES ('".$datestamp."', '4', 'PM', '".$char['username']."', '".$message4."')");
                	}else{
	                    $message3 = "<font color=\'#FF7700\'>From </font><a href=\'javascript:toptell(\"".$char['username']."\");\'><font color=\'#FF7700\' style=\'text-decoration:none\'>".$charname."</font></a><font color=\'#FF7700\'>: ".$message2." [".$date."]</font><br />";
	                    $message4 = "<font color=\'#FF7700\'>To </font><a href=\'javascript:toptell(\"".$touser."\");\'><font color=\'#FF7700\' style=\'text-decoration:none\'>".$touser."</font></a><font color=\'#FF7700\'>: ".$message2." [".$date."]</font><br />";
	
	                    $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES ('".$datestamp."', '4', 'PM', '".$touser."', '".$message3."')");
        ...
        elseif(substr($message, 0, 13) == "/castaffinity")
        {
            ...
            if($caststr[1] != NULL && $caststr[2] != NULL){
            	$findCharacter = mysqli_query($login, "SELECT * FROM characters WHERE username='".$caststr[1]."'");
            		...
            		if($caststr[2] == "Might" || $caststr[2] == "Might II" || $caststr[2] == "Might III" || $caststr[2] == "Might IV" || $caststr[2] == "Might V" || $caststr[2] == "Speed" || $caststr[2] == "Speed II" || $caststr[2] == "Speed III" || $caststr[2] == "Speed IV" || $caststr[2] == "Speed V" || $caststr[2] == "Constitution" || $caststr[2] == "Constitution II" || $caststr[2] == "Constitution III" || $caststr[2] == "Constitution IV" || $caststr[2] == "Constitution V" || $caststr[2] == "Intelligence" || $caststr[2] == "Intelligence II" || $caststr[2] == "Intelligence III" || $caststr[2] == "Intelligence IV" || $caststr[2] == "Intelligence V" || $caststr[2] == "Concentration" || $caststr[2] == "Concentration II" || $caststr[2] == "Concentration III" || $caststr[2] == "Concentration IV" || $caststr[2] == "Concentration V"){
            			if(in_array($caststr[2], $spell)){
            				...
            				if($char['mana'] >= $manaCost){
            					...
            					$giveUserAffinity = mysqli_query($login, "UPDATE characters SET blessing='".$newBlessingString."' WHERE username='".$caststr[1]."'");
            					$subtractMana = mysqli_query($login, "UPDATE characters SET mana=mana-'".$manaCost."' WHERE id='".$char['id']."'");
								...
								$query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `to`, `message`) VALUES ('".$datestamp."', '4', 'PM', '".$castto['username']."', '".$message."')");
        ...
        {
            $query = mysqli_query($login, "SELECT * FROM muted WHERE username='".$char['username']."'") or die(mysqli_error($login));
            ...
            else
            {
            	...
            	$preGetGuild = mysqli_query($login, "SELECT * FROM guilds WHERE name='".$char['guild']."'");
                ...
                $userStatus = "".$getGuildTag."-<font color=\'red\'>Moderator</font>";
                    if($char['chatcolour'] == "00DDDD"){
                     $modupdate = mysqli_query($login, "UPDATE characters SET chatcolour='BBBBBB' WHERE username='".$char['username']."'");
                    }
            ...
            $query = mysqli_query($login, "UPDATE characters SET lastactive='".$date."' WHERE username='".$char['username']."' ") or die(mysqli_error($login));
        }
    }
}
else
