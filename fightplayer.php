...
$findDuel = mysqli_query("SELECT * FROM duelground WHERE `tousername`='".$char['username']."' OR `fromusername`='".$char['username']."'");
if(mysqli_num_rows($findDuel) == 0){
	...
	if($duel['status'] == "Requesting" || $duel['turn'] != $char['username']){
		...
		if($charlife <= "0"){
			...
			$updateDuel = mysqli_query("DELETE FROM duelground WHERE `id`='".$duel['id']."'");
		}else{
			if($char['username'] == $duel['tousername']){
				$findOponent = mysqli_query("SELECT * FROM characters WHERE username='".$duel['fromusername']."'");
				$oponent = mysqli_fetch_assoc($findOponent);
			}else{
				$findOponent = mysqli_query("SELECT * FROM characters WHERE username='".$duel['tousername']."'");
				$oponent = mysqli_fetch_assoc($findOponent);
			}
			...
			if($oponentlife <= "0"){
				...
				if($initnumber > $initvalue){ //You missed
					...
					$query = mysqli_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '4', 'PM', '".$messagechat."', '".$char['username']."')");
					...
					$query = mysqli_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '4', 'PM', '".$messagechat."', '".$oponentname."')");
					
					$updateDuel = mysqli_query("UPDATE duelground SET turn='".$oponentname."', time='".$date."' WHERE id='".$duel['id']."'");
                	...
                	if($oponentlife <= "0"){ //oponent is dead
						...
						$updateRatio = mysqli_query("UPDATE characters SET duelratio='".$newRatio."' WHERE username='".$char['username']."'");
						...
						$updateRatio = mysqli_query("UPDATE characters SET duelratio='".$newRatio."' WHERE username='".$oponent['username']."'");
						
						$endDuel = mysqli_query("DELETE FROM duelground WHERE id='".$duel['id']."'");
						...
						$changeDuelSides = mysqli_query("UPDATE duelground SET turn='".$oponentname."', time='".$date."' WHERE id='".$duel['id']."'");
                	}
                	
                	$updateOponent = mysqli_query("UPDATE characters SET life='".$oponentlife."' WHERE username='".$oponentname."'");
				}
