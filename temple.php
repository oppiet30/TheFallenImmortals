...

{
...

    {
        ...
        $update = db_query("UPDATE characters SET gold=?, temple='1' WHERE id=?", [$byegold, $_SESSION['userid']]);

        $add = $temple['pot'] + $amount;

        $moregold = db_query("UPDATE temple SET pot=?", [$add]);

        $addUserToLotto = db_query("INSERT INTO donationpot (`username`) VALUES (?)", [$char['username']]);
        ...
        if($amount >= 250000 || $char['level'] < "250"){
            ...
            $numberBlessings = db_num_rows(db_query("SELECT * FROM `affinity` WHERE name<>'None' AND name<>'Buy'"));             
            ...
            if($random01 == 1 && $char['affinitys'] >= '1'){
                ...
                $name = db_fetch_assoc(db_query("SELECT * FROM affinity WHERE id=?", [$firstSlot]));
            ...
            if($random02 == 1 && $char['affinitys'] >= '2'){
                ...
                $name = db_fetch_assoc(db_query("SELECT * FROM affinity WHERE id=?", [$secondSlot]));
            ...
            if($random03 == 1 && $char['affinitys'] >= '3'){
                ...
                $name = db_fetch_assoc(db_query("SELECT * FROM affinity WHERE id=?", [$thirdSlot]));
            ...
            if($random04 == 1 && $char['affinitys'] >= '4'){
                ...
                $name = db_fetch_assoc(db_query("SELECT * FROM affinity WHERE id=?", [$fourthSlot]));
            ...
            if($random05 == 1 && $char['affinitys'] >= '5'){
                ...
                $name = db_fetch_assoc(db_query("SELECT * FROM affinity WHERE id=?", [$fifthSlot]));
            ...
            $blessingsINmysql = db_query("UPDATE characters SET blessing=? WHERE id=?", [$updateBlessings, $_SESSION['userid']]);
		...
		if($amount >= 5000000){
            ...
            $increaseTradeSkill = db_query("UPDATE characters SET nobility=? WHERE id=?", [$newNp, $_SESSION['userid']]);
...

{
...

    {
		...
		$findDonations = db_query("SELECT * FROM donationpot WHERE username=?", [$char['username']]);
		$timesDonated = db_num_rows($findDonations);
