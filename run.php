...
if($char['life'] <= "0"){
	...
	$updatemonster = db_query("UPDATE characters SET enemylife='0' WHERE id=?", [$_SESSION['userid']]);
...
$getSmallDemons = db_query("SELECT * FROM demons WHERE health>'0' AND power='1'");
if(db_num_rows($getSmallDemons) > "0" && $char['level'] < '10000'){
    while($demon = db_fetch_array($getSmallDemons)){
        $display .= "".$demon['name']."/ Health:".number_format($demon['health'])." (".$demon['xpos'].", ".$demon['ypos'].")<br />";
    }
}
$getBigDemons = db_query("SELECT * FROM demons WHERE health>'0' AND power='2'");
if(db_num_rows($getBigDemons) > "0"){
    while($demon = db_fetch_array($getBigDemons)){
        $display .= "".$demon['name']."/ Health:".number_format($demon['health'])." (".$demon['xpos'].", ".$demon['ypos'].")<br />";
    }
}
print("fillDiv('displayArea','".$display."');");
