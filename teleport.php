...
if($char['teleporter'] == "Yes"){
	...
	if($xcord <= "100" && $xcord >= "1" && $ycord <= "100" && $ycord >= "1"){
		...
		$timeTeleported = time();
		$updateLocation = db_query("UPDATE characters SET posx=?, posy=?, teleportlast=? WHERE username=?", [$xcord, $ycord, $timeTeleported, $char['username']]);
		$findMap = db_query("SELECT * FROM map WHERE xpos=? and ypos=?", [$xcord, $ycord]);
