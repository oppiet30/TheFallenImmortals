...
if($char['teleporter'] == "Yes"){
	...
	if($xcord <= "100" && $xcord >= "1" && $ycord <= "100" && $ycord >= "1"){
		...
		$timeTeleported = time();
		$updateLocation = mysqli_query($login, "UPDATE characters SET posx='".$xcord."', posy='".$ycord."', teleportlast='".$timeTeleported."' WHERE username='".$char['username']."'");
		$findMap = mysqli_query($login, "SELECT * FROM map WHERE xpos='".$xcord."' and ypos='".$ycord."'");
