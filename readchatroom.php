...
if($char['endsuspend'] < $activeTime && $char['endsuspend'] > "0"){
    ...
    $date = date('ymdHi');
	$whatTime = time();
    $setstatus = mysqli_query($login, "UPDATE characters SET status='Normal', lastactive='".$whatTime."', endsuspend='0', reason='None' WHERE username='".$char['username']."'");
	
	$query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`)VALUES ('".$date."', '3', '".$char['username']."', '".$suspendmessage."', 'Chatroom')") or die(mysqli_error($login));
}
...
$findOfflineMessages = mysqli_query($login, $finder);
...
if(mysqli_num_rows($findOfflineMessages) > 1 && $activeTime < $charLastActive){
	...
	$message .= "".mysqli_real_escape_string($login, $findMsg['message'])."";

	$activeTime = time();

	$query = mysqli_query($login, "INSERT INTO `chatroom` (`date`, `userlevel`, `username`, `to`, `message`) VALUES ('".$activeTime."', '4', 'PM', '".$char['username']."', '".mysqli_real_escape_string($login, $message)."')");

	$deleteTehFreakingMessage = mysqli_query($login, "DELETE FROM `chatroommessage` WHERE `to`='".$char['username']."'");
...
$findYourDuel = mysqli_query($login, "SELECT * FROM duelground WHERE `fromusername`='".$char['username']."'");
...
if($timeofDuel < $date && $char['username'] != NULL && $duel['fromusername'] != NULL){
    ...
    $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '4', 'PM', '".$messagechat."', '".$duel['fromusername']."')");
    ...
    $query = mysqli_query($login, "INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '4', 'PM', '".$messagechat."', '".$duel['tousername']."')");

    

    $deleteDuelRequest = mysqli_query($login, "DELETE FROM duelground WHERE `id`='".$duel['id']."'");
...

    {
        ...
        $getmessages = mysqli_query($login, "SELECT * FROM chatroom WHERE `to`='".$char['username']."' OR `to`='Admin' OR `to`='Mod' OR `to`='Chatroom' OR `username`='".$char['username']."' AND id>'".$char['chatlog']."' ORDER BY id DESC LIMIT 40");
...

    {
        ...
        $getmessages = mysqli_query($login, "SELECT * FROM chatroom WHERE `to`='".$char['username']."' OR `to`='Mod' OR `to`='Chatroom' OR `username`='".$char['username']."' AND id>'".$char['chatlog']."' ORDER BY id DESC LIMIT 40");
...

    {
        ...
        $getmessages = mysqli_query($login, "SELECT * FROM chatroom WHERE `to`='".$char['username']."' OR `to`='Chatroom' OR `username`='".$char['username']."' AND id>'".$char['chatlog']."' ORDER BY id DESC LIMIT 20");
    ...
    $findonline = mysqli_query($login, "SELECT * FROM characters WHERE lastactive>'".$time."' ORDER BY userlevel, id");
...

    {
		...
		$findGuildTag = mysqli_query($login, "SELECT * FROM guilds WHERE name='".$active['guild']."'");
