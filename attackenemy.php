...
$Fixplayers = mysqli_query("UPDATE characters SET captcha='Inactive', captcha_time_limit='0'");
...
if($char['security']=="1"){
    $setback = mysqli_query("UPDATE characters SET security='0' WHERE id='".$char['id']."'");
}

$securitytest = rand(1,200);
if($securitytest == "1" && $char['auto'] == "0"){
	$giveTest = mysqli_query("UPDATE characters SET security='1' WHERE id='".$char['id']."'");
}
...
$getenemy = mysqli_query("SELECT * FROM enemies WHERE id='".$enemyid."'") or die(mysqli_error());
...
/*$getlocation = mysqli_query("SELECT * FROM areas WHERE name='".$charloc."'");
...
if($enemylife > "0"){
	...
	if($charlife > "0"){    //Player isn't dead
        ...
        $setlife = mysqli_query("UPDATE characters SET life='".$charlife."' WHERE id='".$_SESSION['userid']."' ");
        if($enemylife < "1")    //Enemy has died and the player is rewarded for the fight
        {
            if($charguild != "None")
            {
		$getMonsterRows = mysqli_query("SELECT * FROM enemies WHERE level <='".$enemylvl."'");
		$gettingMR = mysqli_num_rows($getMonsterRows);
				...
				$addGuildGold = mysqli_query("UPDATE guilds SET bank=bank+'".$tax."' WHERE name='".$charguild."'");
		...
		$gettingMR = mysqli_num_rows($getMonsterRows);
			...
			$getBonusTime = mysqli_query("SELECT * FROM bonus WHERE id='1'");
			...
			if($bonusTime['experationTime'] <= $date && $bonusTime['experationTime'] != 0){
				$killBonusTime = mysqli_query("UPDATE bonus SET gold='No', experience='No', experationTime='0' WHERE id='1'");
			...
			$lastFight = time();
            $rewards = mysqli_query("UPDATE characters SET expacq='".$newexp."', killstreak='".$newkillstreak."', gold='".$newgold."', blood='".$newblood."', enemylife='0', lastfight='".$lastFight."' WHERE id='".$_SESSION['userid']."' ");
            ...
            if($char['auto'] > "0"){
	            $charauto --;
				$query = mysqli_query("UPDATE characters SET auto='".$charauto."' WHERE id='".$_SESSION['userid']."'");
            }
            ...
            if($newexp >= $chartnl)    //Player has levelled up and has gained some stats
            {
                ...
                $getclass = mysqli_query("SELECT * FROM classes WHERE name='".$charclass."'");
                ...
                $setchar = mysqli_query("UPDATE characters SET strength='".$newstr."', dexterity='".$newdex."', endurance='".$newend."', intelligence='".$newint."', concentration='".$newcon."', stats='".$newstats."', level='".$newlvl."', expacq='".$newexp."', expreq='".$newtnl."' WHERE id='".$_SESSION['userid']."'");
            }
			...
			$findAdventure = mysqli_query("SELECT * FROM scavenger WHERE username='".$char['username']."' AND location='".$location."' AND monster='".$enemyname."'");
			...
			if(mysqli_num_rows($findAdventure) > 0 && $collect[0] < $collect[1]){
				...
				if($drop <= $dropNeeded)    //Adventure item drop
            	{
            		...
            		$updateCollect = mysqli_query("UPDATE scavenger SET collect='".$collectcorrection."' WHERE id='".$advent['id']."'");
			...
			$findCash = mysqli_query("SELECT * FROM cashpot WHERE cash>'0'");
            ...
            if($drop <= $base && $cash['cash'] > 0 && $char['networth'] > 0)    //Cash Drop
            {
                ...
                $cash = $cash['cash'] - 1;
                $addcash = mysqli_query("UPDATE characters SET cash=cash+'1' WHERE id='".$_SESSION['userid']."'");
                $removecash = mysqli_query("UPDATE cashpot SET cash=cash-'1'");
            ...
            if($drop <= "40")    //Statpoint Drop
            {
                ...
                $addpoints = mysqli_query("UPDATE characters SET stats='".$charstats."' WHERE id='".$_SESSION['userid']."'");
            ...
            if($drop <= "40")    //Gold Drop
            {
				...
				print("fillDiv('rewardPopup','".$fillImg."');");
                $addgold = mysqli_query("UPDATE characters SET gold='".$chargold."' WHERE id='".$_SESSION['userid']."'");
            ...
            if($drop <= "40")    //Blood Drop
            {
				...
				print("fillDiv('rewardPopup','".$fillImg."');");
                $addgold = mysqli_query("UPDATE characters SET blood='".$charblood."' WHERE id='".$_SESSION['userid']."'");
            ...
            if($drop <= "40")    //bag Drop
            {
                $randx = rand(1,100);
                $randy = rand(1,100);
                $addBag = mysqli_query("INSERT INTO bagdrop(`name`, `posx`, `posy`) VALUES ('Bag', '".$randx."', '".$randy."')");
            ...
            if($drop <= "20" && $char['mana'] < $char['intelligence'])    //mana Drop
            {
				...
				print("fillDiv('rewardPopup','".$fillImg."');");
                $addBag = mysqli_query("UPDATE characters SET mana='".$char['intelligence']."' WHERE id='".$_SESSION['userid']."'");
            ...
            if($drop <= "10" && $char['temple'] == "1")    //Temple Regenerate
            {
				...
				print("fillDiv('rewardPopup','".$fillImg."');");
                $addBag = mysqli_query("UPDATE characters SET temple='0' WHERE id='".$_SESSION['userid']."'");
            ...
            if($drop <= "3" && $char['tradeskill'] < "1000")    //Trade Skill Increase
            {
				...
				print("fillDiv('rewardPopup','".$fillImg."');");
                $increaseTradeSkill = mysqli_query("UPDATE characters SET tradeskill=tradeskill+'1' WHERE id='".$_SESSION['userid']."'");
            }
			$drop = mt_rand("1","100000");
            if($drop <= "3")    //Cash Drop
            {
				...
				print("fillDiv('rewardPopup','".$fillImg."');");
                $increaseTradeSkill = mysqli_query("UPDATE characters SET cash=cash+'1', nobility='".$newNp."' WHERE id='".$_SESSION['userid']."'");
            ...
            if($drop <= "5")    //Demon Spawn
            {
                ...
                $query = mysqli_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '3', '".$char['username']."', '".$messagechat."', 'Chatroom')");
                $spawnDemon = mysqli_query("INSERT INTO demons (`name`, `health`, `power`, `xpos`, `ypos`) VALUES ('".$BossName."', '".$BossHealth."', '".$BossPower."', '".$randBossX."', '".$randBossY."')");
            }
        ...
        elseif($charlife < "1")    //Player has died
        {
            ...
            $killplayer = mysqli_query("UPDATE characters SET life='0', auto='0', enemylife='".$enemylife."' WHERE id='".$_SESSION['userid']."' ");
			print("fillDiv('displayArea','".$data."');");
			die();
        }
        else    //Neither are dead and the form is given to attack again
        {
            $damageplayer = mysqli_query("UPDATE characters SET life='".$charlife."', enemylife='".$enemylife."' WHERE id='".$_SESSION['userid']."' ");
