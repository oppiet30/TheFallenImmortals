...
if($_POST['ore'] != NULL || $_POST['ore'] != "" || $_POST['ore'] != " "){
	...
	$intelligence = "0";
	$levelreq = "0";
	$findForgeItems = mysqli_query($login, "SELECT * FROM forge WHERE username='".$char['username']."'");
	if(mysqli_num_rows($findForgeItems) == "4"){
		while($inventory = mysqli_fetch_array($findForgeItems)){
			...
			if($_POST['ore'] == "copper" && $char['copperore'] >= 50){
				$updateResources = mysqli_query($login, "UPDATE characters SET copperore=copperore-'50' WHERE username='".$char['username']."'");
			}elseif($_POST['ore'] == "iron" && $char['copperore'] >= 100 && $char['ironore'] >= 50){
				$updateResources = mysqli_query($login, "UPDATE characters SET copperore=copperore-'100', ironore=ironore-'50' WHERE username='".$char['username']."'");
			}elseif($_POST['ore'] == "steel" && $char['copperore'] >= 150 && $char['ironore'] >= 100 && $char['steelore'] >= 50){
				$updateResources = mysqli_query($login, "UPDATE characters SET copperore=copperore-'150', ironore=ironore-'100', steelore=steelore-'50' WHERE username='".$char['username']."'");
			}else{
				print"alert('Not enough resources.');";
				die();
			}
				
				$addToInventory = mysqli_query($login, "INSERT INTO `inventory` (`username`, `itemname`, `equipped`, `levelreq`, `type`, `power`, `strength`, `dexterity`, `endurance`, `intelligence`, `concentration`, `value`) VALUES ('".$char['username']."', '".$itemname."', 'No', '".$levelreq."', '".$itemtype."', '".$power."', '".$strength."', '".$dexterity."', '".$endurance."', '".$intelligence."', '".$concentration."', '".$value."')");
	$removeFromInventory = mysqli_query($login, "DELETE FROM forge WHERE username='".$char['username']."'");
