...
$findCharacters = mysqli_query($login, "SELECT * FROM characters WHERE subscribed='Yes'");

while($whom = mysqli_fetch_assoc($findCharacters)){
	...
	if($whom['activated'] != "Yes"){
		$findActivationCode = mysqli_query($login, "SELECT * FROM activation WHERE username='".$whom['username']."'");
		$open = mysqli_fetch_assoc($findActivationCode);
