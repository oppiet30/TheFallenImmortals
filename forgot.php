...
$email = $_POST['email'];
	$findEmailAssoc = mysqli_query($login, "SELECT * FROM characters WHERE email='".$email."'");
	if(mysqli_num_rows($findEmailAssoc) == 1){
		...
		$createTempPass = mysqli_query($login, "UPDATE characters SET temppass='".$hashedTemp."' WHERE email='".$email."'")or die();
