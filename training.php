...
if(isset($_POST['train']) && $_POST['train'] == $char['class'] && $char['class'] == "Fighter")
{
    if($char['gold'] >= "500000" && $char['blood'] >= "5000" && $char['level'] >= "500"){
        ...
        $updatecharacter = db_query("UPDATE characters SET gold=?, class=?, blood=? WHERE id=?", [$newgold, $newclass, $newblood, $_SESSION['userid']]);
    	...
    	$updateclass = db_query("UPDATE characters SET classlevel=classlevel+'1' WHERE id=?", [$char['id']]);
    }else{
...
else
{
    ...
    $findOtherClass = db_query("SELECT * FROM secondclass WHERE username=?", [$char['username']]);
    $otherClass = db_fetch_assoc($findOtherClass);;
