...
if($_POST['change'] == "yes" && $char['gold'] >= "100000000" && $char['level'] >= "5000"){
	...
	$updateCharacter = mysqli_query($conn, "UPDATE characters SET gold=gold-'100000000' WHERE id='".$char['id']."'");
	$changeClass = mysqli_query($conn, "UPDATE characters SET level='".$sClass['level']."', class='".$sClass['class']."', expacq='".$sClass['expacq']."', expreq='".$sClass['expreq']."', blood='".$sClass['blood']."' WHERE id='".$char['id']."'");
	$storeClass = mysqli_query($conn, "UPDATE secondclass SET level='".$CharLevel."', class='".$CharClass."', expacq='".$CharExpacq."', expreq='".$CharExpreq."', blood='".$CharBlood."' WHERE username='".$char['username']."'");
