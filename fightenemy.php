...
if($_POST['auto'] == "Yes")
{
	...
	if($randomCAPTCHA == 1 && $char['auto'] == 0 && $char['username']=="Ajezior"){
		...
		$setupCaptcha = mysqli_query("UPDATE characters SET auto='0', captcha='Active', captcha_time_limit='".$amount_of_time."' WHERE username='".$char['username']."'");
		...
		if($char['auto'] > "0"){
			...
			$query = mysqli_query("UPDATE characters SET auto='".$char['automax']."', lastfight='0' WHERE id='".$_SESSION['userid']."'");
		}
	}
}elseif($char['auto'] == "0"){
...
$getenemy = mysqli_query("SELECT * FROM enemies WHERE id='".$_POST['enemyid']."'");
...
$updatechar = mysqli_query("UPDATE characters SET enemyid='".$enemyid."', enemylife='".$enemylife."', lastactive='".$date."' WHERE id='".$_SESSION['userid']."'");
