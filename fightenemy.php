...
if($_POST['auto'] == "Yes")
{
	...
	if($randomCAPTCHA == 1 && $char['auto'] == 0 && $char['username']=="Ajezior"){
		...
		if($char['auto'] > "0"){
			...
			$query = db_query("UPDATE characters SET auto=?, lastfight='0' WHERE id=?", [$char['automax'], $_SESSION['userid']]);
		}
	}
}elseif($char['auto'] == "0"){
...
$getenemy = db_query("SELECT * FROM enemies WHERE id=?", [$_POST['enemyid']]);
...
$updatechar = db_query("UPDATE characters SET enemyid=?, enemylife=?, lastactive=? WHERE id=?", [$enemyid, $enemylife, $date, $_SESSION['userid']]);
