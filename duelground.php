...
if($char['life'] > 0){
	if($char['posx'] == "25" && $char['posy'] == "25"){
		...
		$time = time() - "600";
		$findCharacters = db_query("SELECT * FROM characters where level>'100' AND lastactive>? AND username<>? ORDER BY level", [$time, $char['username']]);
		$data .= "<table>";
		$data .= "<tr><td>Username</td><td colspan=\'2\'>Level</td></tr>";
		while($duel = db_fetch_array($findCharacters)){
