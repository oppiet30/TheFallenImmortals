...
if(isset($_POST['decision']) && $_POST['decision'] == "Yes"){
    ...
    if($char['username'] == $guild['leader']){
        if($guild['coleader'] != Null){
            ...
            $promoteColeader = mysqli_query("UPDATE guilds SET leader='".$guild['coleader']."', coleader='' WHERE name='".$guild['name']."'");
        }elseif($guild['captain'] != Null){
            $data .= "You have left your guild promoting ".$guild['captain']." to leader.<br />";
            $promoteCaptain = mysqli_query("UPDATE guilds SET leader='".$guild['captain']."', captain='' WHERE name='".$guild['name']."'");
        }else{
            $guildname = $guild['name'];
            ...
            $query = mysqli_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('', '3', '".$char['username']."', '".$messagechat."', 'Chatroom')");
            $removeUsers = mysqli_query("UPDATE characters SET guild='None', totaldonations='0' WHERE guild='".$charguild."'");
            $goodbyeGuild = mysqli_query("DELETE FROM guilds WHERE name='".$guildname."'");
            $goodbyeLog = mysqli_query("DELETE FROM log WHERE name='".$guildname."'");
        }
    }elseif($char['username'] == $guild['coleader']){
        $data .= "You have left your guild.<br />";
        $promoteCaptain = mysqli_query("UPDATE guilds SET coleader='' WHERE name='".$guild['name']."'");
    }elseif($char['username'] == $guild['captain']){
        $data .= "You have left your guild.<br />";
        $promoteCaptain = mysqli_query("UPDATE guilds SET captain='' WHERE name='".$guild['name']."'");
    }
    $logMessage = "".$char['username']." left the guild!";
    $letGuildKnow = mysqli_query("INSERT INTO log (`name`, `message`) VALUES ('".$charguild."', '".$logMessage."')");
    $updateCharacter = mysqli_query("UPDATE characters SET guild='None', totaldonations='0' WHERE username='".$char['username']."'");
