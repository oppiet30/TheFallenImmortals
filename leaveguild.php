...
if(isset($_POST['decision']) && $_POST['decision'] == "Yes"){
    ...
    if($char['username'] == $guild['leader']){
        if($guild['coleader'] != Null){
            ...
            $promoteColeader = mysqli_query($conn, "UPDATE guilds SET leader='".$guild['coleader']."', coleader='' WHERE name='".$guild['name']."'");
        }elseif($guild['captain'] != Null){
            $data .= "You have left your guild promoting ".$guild['captain']." to leader.<br />";
            $promoteCaptain = mysqli_query($conn, "UPDATE guilds SET leader='".$guild['captain']."', captain='' WHERE name='".$guild['name']."'");
        }else{
            $guildname = $guild['name'];
            ...
            $removeUsers = mysqli_query($conn, "UPDATE characters SET guild='None', totaldonations='0' WHERE guild='".$charguild."'");
            $goodbyeGuild = mysqli_query($conn, "DELETE FROM guilds WHERE name='".$guildname."'");
            $goodbyeLog = mysqli_query($conn, "DELETE FROM log WHERE name='".$guildname."'");
        }
    }elseif($char['username'] == $guild['coleader']){
        $data .= "You have left your guild.<br />";
        $promoteCaptain = mysqli_query($conn, "UPDATE guilds SET coleader='' WHERE name='".$guild['name']."'");
    }elseif($char['username'] == $guild['captain']){
        $data .= "You have left your guild.<br />";
        $promoteCaptain = mysqli_query($conn, "UPDATE guilds SET captain='' WHERE name='".$guild['name']."'");
    }
    ...
    $updateCharacter = mysqli_query($conn, "UPDATE characters SET guild='None', totaldonations='0' WHERE username='".$char['username']."'");
