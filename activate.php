...
if($_GET['key'] != "")
{
	$findkey = mysqli_query($login, "SELECT * FROM activation WHERE `key`='".$_GET['key']."'") or die(mysqli_error($login));
	if(mysqli_num_rows($findkey) == "1")
	{
		...
		$deletekey = mysqli_query($login, "DELETE FROM activation WHERE `key`='".$_GET['key']."'") or die(mysqli_error($login));
		$activatechar = mysqli_query($login, "UPDATE characters SET activated='Yes' WHERE username='".$key['username']."'");
	}
	else
	{
