...
if($_POST['charrequesting'] != NULL){
	$currentDuel = mysqli_query("SELECT * FROM duelground WHERE tousername='".$char['username']."' OR fromusername='".$char['username']."'");
	if(mysqli_num_rows($currentDuel) >= 1){
		print("alert('You already have a pending duel request. Check the chatroom.');");
	}else{
		$findOponent = mysqli_query("SELECT * FROM characters WHERE username='".$_POST['charrequesting']."'")or die(mysqli_error());
		if(mysqli_num_rows($findOponent) == 1){
			$oponent = mysqli_fetch_assoc($findOponent);
			if($oponent['life'] > 0){
				$date = time();
				$addDuelPending = mysqli_query("INSERT INTO duelground(`fromusername`, `tousername`, `status`, `turn`, `time`) VALUES ('".$char['username']."', '".$oponent['username']."', 'Requesting', '".$char['username']."', '".$date."')");
				...
				$query = mysqli_query("INSERT INTO chatroom (`date`, `userlevel`, `username`, `message`, `to`) VALUES ('".$date."', '4', 'PM', '".$messagechat."', '".$oponent['username']."')");
			}else{
