...
if(!ctype_digit($itemid)){
	...
	$toast = mysqli_query($login, "INSERT INTO hackreportlog (`information`) VALUES ('".$char['username']." has attempted to alter equipment ID while equipping. The ID in inventory that was targeted was: ".$itemid."')");
	die();
}
$getitem = mysqli_query($login, "SELECT * FROM inventory WHERE id='".$itemid."' AND username='".$char['username']."'");
if(mysqli_num_rows($getitem) == "1")    //Item exists
{
    ...
    if($charname == $item['username'])    //Item belongs to manipulating player
    {
        if($charlevel >= $item['levelreq'])    //High enough level to equip
        {
            ...
            $accommodationUpdate = mysqli_query($login, "UPDATE characters SET equipped='".$accommodations."' WHERE id='".$_SESSION['userid']."'") or die(mysqli_error($login));
            $unequip = mysqli_query($login, "UPDATE inventory SET equipped='No' WHERE type='".$item['type']."' AND username='".$charname."'");
            $equip = mysqli_query($login, "UPDATE inventory SET equipped='Yes' WHERE id='".$itemid."'");
            print("viewInventory();");
