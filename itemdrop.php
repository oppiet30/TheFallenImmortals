...
if($itemname == ""){
	...
	$findInventory = db_query("SELECT * FROM inventory WHERE username=?", [$char['username']]);
	$inventoryFull = db_num_rows($findInventory);
	if($inventoryFull >= $char['backpacksize']){
		...
		$activeTime = time();
		$query = db_query("INSERT INTO `chatroom` (`date`, `userlevel`, `username`, `to`, `message`) VALUES (?, '4', 'PM', ?, ?)", [$activeTime, $char['username'], $data2]);
	}else{
		$makeitem = db_query("INSERT INTO inventory (`username`, `itemname`, `levelreq`, `power`, `type`, `strength`, `dexterity`, `endurance`, `intelligence`, `concentration`, `value`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [$charname, $itemname, $bonmin, $itempower, $itemtype, $strbon, $dexbon, $endbon, $intbon, $conbon, $value]);

		$itemSet = db_insert_id();
