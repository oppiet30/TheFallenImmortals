...
if($itemname == ""){
	...
	$findInventory = mysqli_query("SELECT * FROM inventory WHERE username='".$char['username']."'");
	...
	if($inventoryFull >= $char['backpacksize']){
		...
		$activeTime = time();
		$query = mysqli_query("INSERT INTO `chatroom` (`date`, `userlevel`, `username`, `to`, `message`) VALUES ('".$activeTime."', '4', 'PM', '".$char['username']."', '".mysqli_real_escape_string($data2)."')");
	}else{
		$makeitem = mysqli_query("INSERT INTO inventory (`username`, `itemname`, `levelreq`, `power`, `type`, `strength`, `dexterity`, `endurance`, `intelligence`, `concentration`, `value`) VALUES ('".$charname."', '".$itemname."', '".$bonmin."', '".$itempower."', '".$itemtype."', '".$strbon."', '".$dexbon."', '".$endbon."', '".$intbon."', '".$conbon."', '".$value."')");
